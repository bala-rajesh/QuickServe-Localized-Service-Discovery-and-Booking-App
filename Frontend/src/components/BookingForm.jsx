    ...
    const handleSubmit = async () => {
        if (category && address && requestDetails && date && time) {
            if (onSubmit) {
                setLoading(true);
                try {
                    await onSubmit({ category, address, description: requestDetails, date, time });
                } finally {
                    setLoading(false);
                }
            }
                ...
                {step === 1 && (
                    <>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '30px' }}>
                            <button onClick={onCancel} style={{ backgroundColor: '#f1f1f1', color: '#333', padding: '12px 25px', borderRadius: '8px', border: '1px solid #ddd', fontWeight: '600', cursor: 'pointer' }} disabled={loading}>
                                Cancel
                            </button>
                            {initialData ? (
                                <button onClick={handleSubmit} disabled={loading} style={{ backgroundColor: loading ? '#93c5fd' : '#007bff', color: '#fff', padding: '12px 25px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: loading ? 'not-allowed' : 'pointer' }}>
